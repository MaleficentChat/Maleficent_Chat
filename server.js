...
const FEATURE_CONTROLS=[
  {key:"clear_room",name:"Clear room",category:"Rooms & Communities",defaultRank:"COOWNER"},
  {key:"create_room",name:"Create room",category:"Rooms & Communities",defaultRank:"SUPER_ADMIN"},
  {key:"edit_room",name:"Edit room",category:"Rooms & Communities",defaultRank:"COOWNER"},
  {key:"delete_room",name:"Delete room",category:"Rooms & Communities",defaultRank:"COOWNER"},
  {key:"publish_news",name:"Publish news",category:"Admin & Owner Dashboard",defaultRank:"ADMIN"},
  {key:"delete_news",name:"Delete news",category:"Admin & Owner Dashboard",defaultRank:"ADMIN"},
  {key:"moderate_user",name:"Moderate users",category:"Moderation",defaultRank:"MOD"},
  {key:"delete_message",name:"Delete messages",category:"Moderation",defaultRank:"MOD"},
  {key:"profile_like",name:"Like profiles",category:"Social Features",defaultRank:"MEMBER"},
  {key:"profile_edit",name:"Edit own profile",category:"Profile System",defaultRank:"MEMBER"},
  {key:"profile_about",name:"Edit About Me",category:"Profile System",defaultRank:"MEMBER"},
  {key:"friend_request",name:"Send friend requests",category:"Social Features",defaultRank:"MEMBER"},
  {key:"block_user",name:"Block users",category:"Social Features",defaultRank:"MEMBER"},
  {key:"private_message",name:"Private messaging",category:"Messaging",defaultRank:"MEMBER"},
  {key:"send_gift",name:"Send virtual gifts",category:"Social Features",defaultRank:"MEMBER"},
  {key:"upload_profile_media",name:"Upload profile photo/banner",category:"Profile System",defaultRank:"MEMBER"},
  {key:"inspect_private_messages",name:"Inspect private messages",category:"Admin & Owner Dashboard",defaultRank:"OWNER"},
  {key:"manage_ranks",name:"Change user ranks",category:"Ranks & Roles",defaultRank:"OWNER"},
  {key:"manage_gold",name:"Manage user gold",category:"Gamification",defaultRank:"OWNER"},
  {key:"grant_features",name:"Grant feature access",category:"Admin & Owner Dashboard",defaultRank:"OWNER"},
  {key:"owner_space",name:"Owner Space",category:"Admin & Owner Dashboard",defaultRank:"OWNER"}
];
  ...
  (req,res)=>{
    ...
    if(matchedFilter && (rankOrder[u.rank]??999)<=filterMaxRank){
      ...
      db.moderationHistory.push({
        id:id(), time:Date.now(), target:u.id, actor:"SYSTEM", action:"AUTO_MUTE",
        reason:`Filtered word: ${matchedFilter}`, minutes
      });
      save();
      notify(u.id,"moderation","Auto-mute",`Your message matched the room filter and you were muted for ${minutes} minute${minutes===1?"":"s"}.`);
      return res.status(400).json({error:`Message blocked and automatic mute applied for ${minutes} minute${minutes===1?"":"s"}.`});
    }
...
app.get('/api/owner/feature-permissions',ownerOnly,(req,res)=>{
  ensureFeaturePermissions();
  res.json({controls:FEATURE_CONTROLS,permissions:db.featurePermissions,ranks});
});

app.patch('/api/owner/feature-permissions/:key',ownerOnly,(req,res)=>{
  ensureFeaturePermissions();
  const f=FEATURE_CONTROLS.find(x=>x.key===req.params.key);
  ...
  db.featurePermissions[f.key]=r;
  save();
  log(current(req),'FEATURE_PERMISSION_CHANGE',f.key,{rank:r});
  res.json({ok:true,key:f.key,rank:r,permissions:db.featurePermissions});
});
...
app.put('/api/profile',auth,(req,res)=>{
  ...
  const next={displayName:req.body.displayName,bio:req.body.bio,pronouns:req.body.pronouns,relationship:req.body.relationship,mood:req.body.mood,location:req.body.location,website:req.body.website,interests:req.body.interests,profileColor:req.body.profileColor};
...
app.get('/api/users/:id/profile',auth,(req,res)=>{
  ...
  res.json({user:safeUser(v),likedByMe:liked,friendship:friendship?friendship.status:null,profileLikes:v.profileLikes||0});
});
...
app.post('/api/users/:id/gift',auth,featureAllowed('send_gift'),(req,res)=>{
  const u=current(req), v=findUser(req.params.id);
  if(!v || v.id===u.id) return res.status(400).json({error:"Invalid user"});
  const gifts={rose:{name:'Rose',cost:25,icon:'🌹'},star:{name:'Star',cost:50,icon:'⭐'},crown:{name:'Crown',cost:100,icon:'👑'},heart:{name:'Heart',cost:150,icon:'💜'}};
  const g=gifts[req.body.gift]; if(!g) return res.status(400).json({error:"Invalid gift"});
  if((u.gold||0)<g.cost) return res.status(400).json({error:`You need ${g.cost} gold.`});
  u.gold-=g.cost; v.gold=(v.gold||0); v.giftsReceived ||= []; v.giftsReceived.push({from:u.id,gift:g.name,icon:g.icon,time:Date.now()});
  ...
  save(); res.json({ok:true,gift:g,remainingGold:u.gold});
});
