    ...
    if(tab==='gifts'){
      ...
      if(!isSelf){
        api('/api/gifts').then(gd=>{
          ...
          grid.innerHTML=(gd.gifts||[]).map(g=>`<button class="gift-btn" data-gift="${esc(g.id)}"><span>${esc(g.icon)}</span><b>${esc(g.name)}</b><small>🪙 ${g.cost}</small></button>`).join('')||'<p class="muted">No gifts available.</p>';
...
function createCustomGift(){
  modal(`<div class="custom-gift-modal"><h2>✨ Create a custom gift</h2><p class="muted">Make a gift that can be sent from the gift catalog.</p><input id="giftName" maxlength="40" placeholder="Gift name"><input id="giftIcon" maxlength="4" value="🎁" placeholder="Emoji"><input id="giftCost" type="number" min="1" max="100000" placeholder="Gold cost"><div class="toolbar"><button class="primary" id="saveCustomGift">Create gift</button><button class="mini" onclick="closeModal()">Cancel</button></div></div>`);
  $('saveCustomGift').onclick=async()=>{
    try{
      const r=await api('/api/gifts',{method:'POST',body:JSON.stringify({name:$('giftName').value,icon:$('giftIcon').value,cost:$('giftCost').value})});
      closeModal();toast(`${r.gift.icon} ${r.gift.name} created.`);
      if(typeof renderUserProfileModal==="function") renderUserProfileModal(user,{});
    }catch(e){toast(e.message)}
  };
}
