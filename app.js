...
function can(requiredRank) {
  ...
  return (
    (rankOrder[state.user.rank] ?? -1) >=
    (rankOrder[requiredRank] ?? 999)
  );
}
...
function updateNav() {
  ...
  document
    ...
    .forEach(el =>
      el.classList.toggle("hidden", !can("COOWNER"))
    );
}
      ...
      .map(room => {
        ...
        return `
          ...
          >
            ...
            <div class="room-meta">
              ${esc(room.category || "Community")}
              ·
              ${esc(room.rankRequired || "MEMBER")}+
            </div>
          </div>
        `;
      })
...
async function openUser(id) {
  try {
    ...
    const lastSeen = user.privacy?.lastSeen !== false && user.lastSeen
      ? new Date(user.lastSeen).toLocaleString()
      : "Hidden";
    const currentRoom = state.rooms.find(r => r.id === state.currentRoom)?.name || "Main Room";
    const bannerStyle = user.banner
      ? `style="background-image:linear-gradient(180deg,rgba(0,0,0,.05),rgba(0,0,0,.72)),url('${esc(user.banner)}')"`
      : "";
      ...
      <div class="profile-modal">
        <div class="profile-banner" ${bannerStyle}>
          <div class="profile-stats">
            <span class="profile-stat">★ ${user.level || 1}</span>
            <span class="profile-stat">◉ ${user.xp || 0}</span>
          </div>
        ...
        <div class="profile-info">
          ...
          <div class="profile-info-row"><b>🏠 Current room</b><span>${esc(currentRoom)}</span></div>
          <div class="profile-info-row"><b>◉ Last seen</b><span>${esc(lastSeen)}</span></div>
          ${user.pronouns ? `<div class="profile-info-row"><b>⚑ Pronouns</b><span>${esc(user.pronouns)}</span></div>` : ""}

          <div class="profile-actions">
            ${user.id !== state.user.id ? `
              <button class="primary" onclick="openDM('${esc(user.id)}');closeModal()">💬 Private</button>
              <button class="mini" onclick="friend('${esc(user.id)}')">👥 Add Friend</button>
              <button class="mini" onclick="block('${esc(user.id)}')">🚫 Block</button>
            ` : ""}
            ${can("MOD") && user.id !== state.user.id ? `<button class="mini" onclick="moderate('${esc(user.id)}')">🛡️ Moderate</button>` : ""}
          </div>
        </div>
      </div>
    `);
    ...
    >
      ...
      <div class="card">
        ...
        <form id="profileForm">
          ...
          <input
            id="theme"
            value="${esc(user.theme || "")}"
            placeholder="Profile theme"
          >

          <input
            id="pc"
            value="${esc(user.profileColor || "")}"
    ...
    $("staff").innerHTML = `
      <div class="grid2">
        ...
        <div class="card">
          ...
          <p>
            ${
              bootstrapData.settings?.linkFilter
                ? "Link filter ON"
                : "Link filter OFF"
            }
          </p>
...
async function loadSettings(){
  ...
  $('settings').innerHTML=`
    ...
    <div class="panel"><h3>Appearance</h3>
      <select id="v3Theme"><option>obsidian</option><option>midnight</option><option>forest</option><option>rose</option></select>
  ...
  $('saveAppearance').onclick=async()=>{await api('/api/settings',{method:'PUT',body:JSON.stringify({theme:$('v3Theme').value,fontSize:$('v3Font').value,compactMode:$('v3Compact').checked,reducedMotion:$('v3Motion').checked,highContrast:$('v3Contrast').checked})});toast('Appearance saved.');};
